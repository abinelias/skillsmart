...
require 'json'
	...
	def myJobAssessment 
		...
		applied.uniq
		if(applied.include? params[:jobID].to_s)
			@display = "0"
		else
			@display = "1"
		end
				...
				unless(required.blank?)
					required.each do |val|
							...
							if(@skillData.empty?)
								...
								@requiredArray.push("reqSkills" => @name, "reqSkillID" => val, "skillVal" => @proficency, "employeeVal" => 0)																	
							else
								if(h = @skillData.find { |h| h['skillID'] == val })
									...
									@requiredArray.push("reqSkills" => @name, "reqSkillID" => val, "skillVal" => @proficency, "employeeVal" => @prof)																	
				...
				unless(desired.blank?)
					desired.each do |val|
							...
							if(@skillData.empty?)
								...
								@desiredArray.push("desSkills" => @name, "desSkillID" => val, "skillVal" => @proficency, "employeeVal" => 0)																	
							else
								if(h = @skillData.find { |h| h['skillID'] == val })
									h['value'].each do |vlue|
														@prof				=		vlue.proficency
									...
									@desiredArray.push("desSkills" => @name, "desSkillID" => val, "skillVal" => @proficency, "employeeVal" => @prof)																	
